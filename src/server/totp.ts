...
export const totpCode = (secret: string, nowMs = Date.now()) => hotp(base32Decode(secret), totpCounter(nowMs));
...
export function verifyTotp(
  secret: string, code: unknown, opts: { window?: number; nowMs?: number; notBeforeCounter?: number } = {},
): number | null {
  ...
  for (let delta = -window; delta <= window; delta++) {
    ...
    if (opts.notBeforeCounter !== undefined && counter <= opts.notBeforeCounter) continue;
