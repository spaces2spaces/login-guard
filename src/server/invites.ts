...
export const inviteTokenLooksValid = (token: unknown): token is string =>
  typeof token === "string" && INVITE_TOKEN_SHAPE.test(token);

/** Expired, or already used: two ways a link stops working. */
export function inviteIsOpen(invite: { expiresAt: Date; usedAt: Date | null }, now = new Date()): boolean {
